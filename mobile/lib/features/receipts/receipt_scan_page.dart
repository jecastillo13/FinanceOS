...
class _ReceiptScanPageState extends State<ReceiptScanPage> {
  ...
  Future<void> _loadOptions() async {
    try {
      ...
      setState(() {
        _accounts = data[0];
        _categories = data[1]
            .where((item) => '${item['tipo']}'.toLowerCase() == 'gasto')
            .toList();
  ...
  Future<void> _capture() async {
    ...
    try {
      ...
      final firstLine = text.split('\n').map((line) => line.trim()).firstWhere(
            (line) =>
                line.length > 2 &&
                !RegExp(r'factura|nit|total|fecha', caseSensitive: false)
                    .hasMatch(line),
            orElse: () => 'Compra con comprobante',
          );
      final stable =
          '${DateTime.now().toIso8601String().split('T').first}|$total|${text.toLowerCase().replaceAll(RegExp(r'\s+'), ' ').trim()}';
      setState(() {
        _description.text =
  ...
  double? _extractTotal(String text) {
    ...
    final patterns = [
      RegExp(r'total\s*\(\s*cop\s*\)\s*:?\s*\$?\s*([\d.,]+)',
          caseSensitive: false),
    ...
    for (final pattern in patterns) {
      final values = pattern
          ...
          .map((match) => _number(match.group(1)!))
          .whereType<double>()
          .toList();
  ...
  Future<void> _save() async {
    ...
    try {
      final movement = await _api.crearMovimiento(
        ...
        observaciones:
            'Creado desde una factura fotografiada en FinanceOS Mobile.',
        huella: _fingerprint,
      );
      await _api.adjuntarComprobante(
          movimientoId: movement['id'] as int, rutaArchivo: _photo!.path);
        ...
        body: FinanceAurora(
          ...
          FinanceSurface(
              child: Column(
                  ...
                  children: [
                const Text('Captura inteligente',
                    style:
                        TextStyle(fontSize: 22, fontWeight: FontWeight.w900)),
                const SizedBox(height: 7),
                const Text(
                    'Toma una foto, revisa los datos y confirma antes de afectar tu saldo.',
            ...
            FinanceSurface(
              ...
              TextField(
                  ...
                  decoration: const InputDecoration(
                      labelText: 'Comercio o descripción')),
              ...
              DropdownButtonFormField<int>(
                  key: ValueKey('cuenta-$_accountId'),
              ...
              const SizedBox(height: 10),
              DropdownButtonFormField<int>(
                  key: ValueKey('categoria-$_categoryId'),
