    ...
    body: FinanceAurora(child: SafeArea(child: ListView(padding: const EdgeInsets.all(18), children: [
      ...
      if (_photo != null) ...[
        ...
        FinanceSurface(child: Column(children: [
          ...
          const SizedBox(height: 10),
          DropdownButtonFormField<int>(key: ValueKey('cuenta-$_accountId'), initialValue: _accountId, decoration: const InputDecoration(labelText: 'Cuenta'), items: _accounts.map((item) => DropdownMenuItem<int>(value: item['id'] as int, child: Text('${item['nombre']} (${item['moneda']})'))).toList(), onChanged: (value) => setState(() => _accountId = value)),
          const SizedBox(height: 10),
          DropdownButtonFormField<int>(key: ValueKey('categoria-$_categoryId'), initialValue: _categoryId, decoration: const InputDecoration(labelText: 'Categoría'), items: _categories.map((item) => DropdownMenuItem<int>(value: item['id'] as int, child: Text('${item['nombre']}'))).toList(), onChanged: (value) => setState(() => _categoryId = value)),
