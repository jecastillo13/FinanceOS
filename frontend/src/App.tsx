...
const nav = [
  ...
  ["Presupuestos", ChartNoAxesCombined], ["Metas", Target], ["Configuración", Settings],
] as const;
...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    <aside className={`nav-rail ${menu ? "nav-open" : ""}`}>
      ...
      <nav>{nav.map(([label,Icon],index)=><button key={label} className={index===0?"active":""} aria-label={label} title={label}><Icon size={20}/><span>{label}</span>{index===0&&<i/>}</button>)}</nav>
    ...
    <main className="relative z-10 min-h-screen px-4 pb-20 lg:ml-24 lg:px-8 xl:px-12">
      <header className="topbar mx-auto max-w-[1480px]">
        <div className="flex items-center gap-3"><button aria-label="Abrir menú" className="icon-button lg:hidden" onClick={()=>setMenu(true)}><Menu size={20}/></button><div><span className="eyebrow">DOMINGO · 09 AGO</span><h1>Centro financiero</h1></div></div>
      ...
      <div className="mx-auto max-w-[1480px]">
        ...
        <section className="hero-grid">
          <article className="wealth-card">
            ...
            <div className="relative z-10 flex h-full flex-col justify-between">
              ...
              <div><p className="wealth-value">{loading?"—":cop.format(summary?.patrimonio??0)}</p><div className="mt-6 flex flex-wrap gap-3"><button className="primary-action"><Plus size={17}/> Nuevo movimiento</button><button className="ghost-action"><ArrowLeftRight size={17}/> Transferir</button></div></div>
            </div>
          </article>
        ...
        <section className="lower-grid">
          <article className="accounts-panel">
            <div className="panel-heading"><div><span className="eyebrow">LIQUIDEZ</span><h2>Tus cuentas</h2></div><button className="text-action">Explorar <ChevronRight size={15}/></button></div>
