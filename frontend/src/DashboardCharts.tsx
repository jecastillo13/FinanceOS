...
export default function DashboardCharts({ charts, patrimonio }: { charts: Graficas | null; patrimonio: number }) {
  ...
  const debts = useMemo(() => charts?.deudas?.map((item) => ({ ...item, cuenta: item.cuenta.replace(/^\S+\s/, "") })) ?? [], [charts]);
  return <section className="mt-[18px] grid gap-[18px] xl:grid-cols-[1.62fr_1fr]">
    ...
    <article className="chart-panel distribution-panel"><div className="chart-title"><div><span>PORTAFOLIO</span><h3>Distribución</h3><p>Composición del patrimonio</p></div></div>
      ...
      {debts.length>0&&<div className="mt-4 border-t border-white/10 pt-3"><span className="text-xs uppercase tracking-widest text-rose-300/70">Deudas descontadas</span>{debts.slice(0,3).map((item,i)=><div key={`${item.cuenta}-deuda-${i}`} className="mt-2 flex items-center justify-between text-sm"><span className="truncate text-slate-400">{item.cuenta}</span><strong className="text-rose-300">− {cop.format(item.saldo_cop)}</strong></div>)}</div>}
    </article>
  </section>;
}
